...
Import `legalqa_main_04_repair_submit.ipynb` vào Kaggle, chọn GPU T4/P100, bật Internet. Add Input diagnostics Stage 3, output Stage 2/3 có `selected_adapter`, và dataset model/index gốc. Notebook đã có sẵn `MODE='p1_dev'`, `p1_public`, `p2_retrieval`, `p2_generate`, `repair_v2`; không cần thay cell lệnh. Khi có nhiều input, đặt `DIAGNOSTICS`, `MODEL_ROOT`, `ADAPTER_ROOT` cụ thể. Code/scorer/config ablation được nhúng, không cần clone/push GitHub. Hướng dẫn và giới hạn: [docs/main04_v2.md](docs/main04_v2.md).

Mặc định `p1_dev` chỉ sinh lại các câu có vòng lặp, cùng một policy context/prompt và so sánh `repetition_penalty=1.00/1.03/1.05` trên dev. `p1_public` chỉ chấp nhận variant có `passes_screen=true`. `p2_retrieval` tạo năm cache/diagnostic; sau khi chọn tối đa hai tên, `p2_generate` sinh và chấm dev100. Output mới nằm tại `/kaggle/working/legalqa_main_04_v8_060_focused_loop/`; khi paused, gắn đúng output cùng bundle và đặt `PREVIOUS_OUTPUT` để resume.

Trên diagnostics `(5)` ngày 18/09/2026, V1 đạt dev100 METEOR **0,61828 → 0,62025**; người dùng báo public **0,5580 → 0,5599**. GPU recipe 1 đã chạy trên Kaggle nhưng giảm METEOR về **0,61829**, nên giữ CPU. Recipe 2 cũ vẫn dùng được qua `MODE='repair_v2'`; các mode P1/P2 mới chưa được chạy GPU thực và chưa chứng minh đạt public 0,60. Xem [chẩn đoán journal](docs/main04_gpu_journal_review.md).
